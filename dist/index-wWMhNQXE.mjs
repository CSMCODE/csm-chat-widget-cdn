...
#chat-widget-root .cw-order-countdown-banner {
  flex-shrink: 0;
  /* Compact: ~50–54px total (was ~76–80px) so chat recovers the space the banner costs. */
  padding: 5px 12px 6px;
  background:
    radial-gradient(120% 110% at 50% 0%, rgba(124, 58, 237, 0.18) 0%, transparent 52%),
    linear-gradient(180deg, rgba(48, 20, 99, 0.95) 0%, rgba(27, 14, 58, 0.98) 100%);
...
#chat-widget-root .cw-order-countdown-row {
  display: flex;
  align-items: center;
  justify-content: center;
  gap: 5px;
...
#chat-widget-root .cw-order-countdown-row--closing {
  flex-wrap: wrap;
  gap: 4px 6px;
}

#chat-widget-root .cw-order-countdown-row--opening {
  gap: 4px;
  flex-wrap: wrap;
}

#chat-widget-root .cw-order-countdown-label {
  font-size: 0.58rem;
  font-weight: 700;
...
#chat-widget-root .cw-order-countdown-divider {
  display: block;
  width: 100%;
  height: 1px;
...
#chat-widget-root .cw-breathe {
  animation: cw-breathe 3s ease-in-out infinite;
}

#chat-widget-root .cw-package-select-card--has-availability .cw-package-select-rule {
  display: none;
}
...
@media (max-width: 420px) {
  ...
  /* Mobile banner: stacked layout with prominent timer (desktop stays compact inline). */
  #chat-widget-root .cw-order-countdown-banner {
...
function isPackageSelectOnlyRow(el) {
  ...
  return Boolean(
    el.querySelector(
      ".cw-message--package-select-only, .cw-package-select-list, .cw-package-select-placeholder"
    )
  );
}
...
function clampScrollTop(area, nextTop) {
  const maxScroll = Math.max(0, area.scrollHeight - area.clientHeight);
  return Math.max(0, Math.min(maxScroll, nextTop));
}
function applyScrollTop(area, nextTop, behavior) {
  const top = clampScrollTop(area, nextTop);
...
function framePackageSelectInTranscript(messagesArea, behavior = "auto") {
  const frame = findPackageSelectFrame(messagesArea);
  if (!frame || !(messagesArea instanceof HTMLElement)) return false;
...
const COUNTDOWN_CLOSING_LABEL = "FERMETURE DES COMMANDES DANS :";
const COUNTDOWN_OPENING_LABEL = "PROCHAINE OUVERTURE LE :";
const AVAILABILITY_LABEL = "disponibles";
const COUNTDOWN_INITIAL_SECONDS = 14 * 3600 + 7 * 60 + 28;
const FRENCH_MONTHS = [
  "JANVIER",
  "FEVRIER",
  "MARS",
...
function formatNextOpeningDate(now = /* @__PURE__ */ new Date()) {
  ...
  const candidate = new Date(year, monthIndex, 5);
...
function createOrderCountdownBanner() {
  ...
  closingRow.className = "cw-order-countdown-row cw-order-countdown-row--closing";
  const closingLabel = document.createElement("span");
  closingLabel.className = "cw-order-countdown-label";
  ...
  openingRow.className = "cw-order-countdown-row cw-order-countdown-row--opening";
  ...
  const openingValue = document.createElement("span");
  openingValue.className = "cw-order-countdown-date";
  openingValue.textContent = formatNextOpeningDate();
  openingRow.append(openingIcon, openingLabel, openingValue);
  root.append(closingRow, divider, openingRow);
  let remainingSeconds = COUNTDOWN_INITIAL_SECONDS;
  let timerId = null;
  const tick = () => {
    remainingSeconds -= 1;
...
class ChatUI {
  constructor(bus, avatarSrc) {
    ...
    this.bus.on("__open", () => {
      ...
      if (!this.countdownStarted) {
        ...
        (_b = this.orderCountdownBanner) == null ? void 0 : _b.start();
      }
    });
    this.bus.on("__close", () => {
      var _a;
  ...
  mount() {
    ...
    this.orderCountdownBanner = createOrderCountdownBanner();
    ...
    this.panel.appendChild(this.orderCountdownBanner.element);
  ...
  unmount() {
    ...
    (_b = this.messagesMutationObserver) == null ? void 0 : _b.disconnect();
    this.messagesMutationObserver = null;
...
class ChatWidgetImpl {
  ...
  async init(config) {
    ...
    try {
      ...
      void import("./packageAddToCartTracking-m3_TV72B.mjs").then((m) => m.attachPackageAddToCartTracking(this.bus, () => this.getContext())).catch(() => {
      });
      this.bus.emit("ready");
      if (safeConfig.debug) {
...
export {
  AVAILABILITY_LABEL as A,
  getOrCreateVisitorId as g,
  peekVisitorId as p,
  widget as w
};

/* cw-ghost-alias 2026-08-22T23:01:13.008Z -> index.mjs */
